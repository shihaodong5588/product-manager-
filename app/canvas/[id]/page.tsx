...
import { componentTypes } from '@/components/industrial/component-palette'
...
import { ArrowLeft, Save, Trash2, Grid3x3, GitBranch, Wand2, Image as ImageIcon } from 'lucide-react'
import { toast } from 'sonner'
import Link from 'next/link'
import { exportCanvasToBase64, downloadBase64Image } from '@/lib/canvas-export'

export default function EditCanvasPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const fetchDesign = async () => {
    try {
      ...
      if (design.canvasData) {
        setComponents(design.canvasData.components || [])
        setCanvasSettings({
          width: design.canvasData.width || 1920,
          height: design.canvasData.height || 1080,
          gridSize: design.canvasData.gridSize || 20,
          showGrid: design.canvasSettings?.showGrid ?? true,
        })
        setBackgroundImage(design.canvasData.backgroundImage || '')
      }
    } catch (error) {
  ...
  const handleUpdateComponent = useCallback((id: string, updates: Partial<CanvasComponent>) => {
    ...
    // Update selected component as well
  ...
  const handleCreateVersion = async () => {
    try {
      const response = await fetch(`/api/industrial-designs/${id}/version`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          canvasData: {
  ...
  const handleMarkAsFinal = async () => {
    try {
      ...
      setDesignInfo((prev) => ({ ...prev, status: 'final' }))
      toast.success('已标记为最终版本')
    } catch (error) {
      ...
      <div className="border-b bg-card">
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {designInfo.status === 'draft' && (
              <Button variant="outline" size="sm" onClick={handleMarkAsFinal}>
                标记为最终版本
              </Button>
            )}
